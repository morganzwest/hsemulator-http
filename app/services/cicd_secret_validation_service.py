"""
CICD Secret Validation Service for Novocode Runtime
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                logger.info(
                    "CICD token validation successful: token has required scopes")
                return {
                    "status": "valid",
                    "status_code": response.status_code,
                ...
                logger.warning(
                    "CICD token validation failed: invalid token (401)")
                raise CicdTokenInvalidError(
                ...
                logger.warning(
                    "CICD token validation failed: missing scopes (403)")
